...
export default function Header({ user, onLogout }: HeaderProps) {
  ...
  return (
    <>
      <a 
        href="#main-content" 
        className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[100] focus:bg-[#1DB954] focus:text-white focus:px-4 focus:py-2 focus:rounded-lg focus:font-semibold focus:shadow-lg"
      >
      ...
      <header className="border-b border-gray-800 bg-gradient-to-b from-gray-950 to-black backdrop-blur-md sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between">
            <Link href={user ? "/dashboard" : "/login"} className="flex items-center gap-2 sm:gap-3 group">
              <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-lg bg-gradient-to-br from-gray-800 to-gray-700 flex items-center justify-center transition-all group-hover:from-gray-700 group-hover:to-gray-600 border border-gray-700 group-hover:border-gray-600">
                <Music size={20} className="text-white sm:w-5 sm:h-5" strokeWidth={2.5} aria-hidden="true" />
              </div>
        ...
        {user && !isLoginPage && mobileMenuOpen && (
          <>
            ...
            <div 
              id="mobile-menu"
              className="fixed top-[57px] right-0 bottom-0 w-72 bg-gradient-to-b from-gray-950 to-gray-900 border-l border-gray-800 z-50 md:hidden animate-in slide-in-from-right duration-200 shadow-2xl"
              role="dialog"
